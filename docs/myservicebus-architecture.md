...
The current runtime supplies the messaging, correlation, scheduling, outbox, retry, fault, and monitoring primitives from which applications can build workflows. C# and Java can also build, validate, register, and export normalized choreography fragments through topology and inspection metadata. The monitoring service merges those fragments, projects declared contract connections, reconstructs selectable bounded workflow runs, and compares declared outputs with exact causal evidence. Matching C# and Java saga DSLs and executors now run through provider-neutral repository transactions, validate declared correlation, concurrency, durability, outbox, and final-deletion requirements against provider capabilities, register their events on one receive endpoint, dispatch sends and publications through the active consume context, export the normalized state-machine definition plus endpoint attachment, and emit payload-free lifecycle observations after repository commit. The only built-in repository is still volatile. The monitoring service keeps a separate saga definition catalog and bounded instance-transition projection with replica, conflict, state, completion, and delivery-failure information; it does not reinterpret these records as choreography. Shared workflow catalog and run-index queries compose references to both models with an explicit implementation kind and lifecycle authority, and the Dashboard links each entry to its kind-specific declaration or run detail. The mixed Aspire environment demonstrates a C# coordinator with Java inventory and C# payment participants. This is still an experimental orchestration foundation rather than a complete saga feature: no durable saga repository, transactional saga outbox, compensation engine, durable transition history, or rich state-machine graph is supplied yet.
