...
The current runtime supplies the messaging, correlation, scheduling, outbox, retry, fault, and monitoring primitives from which applications can build workflows. C# and Java can also build, validate, register, and export normalized choreography fragments through topology and inspection metadata. The monitoring service merges those fragments, projects declared contract connections, reconstructs selectable bounded workflow runs, and compares declared outputs with exact causal evidence. Matching C# and Java saga DSLs and volatile executors can now register their declared events on one receive endpoint, dispatch sends and publications through the active consume context, and export the normalized state-machine definition plus endpoint attachment. The monitoring service keeps these in a separate saga catalog with replica and definition-conflict information; it does not reinterpret them as choreography. The mixed Aspire environment demonstrates a C# coordinator with Java inventory and C# payment participants. This is still an experimental orchestration foundation rather than a complete saga feature: no durable saga repository, transactional saga outbox, compensation engine, instance-transition monitoring, or Dashboard orchestration view is supplied yet.
