...
Language-specific projections may become more expressive than the reference-client APIs without redefining this boundary. In particular, a future [Raven saga DSL exploration](proposals/raven-saga-dsl.md) may compile concise state-machine declarations into the same portable saga descriptors and runtime behavior. Raven syntax and compiler artifacts remain outside persisted state, wire contracts, and normalized topology, and the experiment follows rather than defines the shared saga semantics.
