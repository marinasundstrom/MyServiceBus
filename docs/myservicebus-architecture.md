...
The current runtime supplies the messaging, correlation, scheduling, outbox, retry, fault, and monitoring primitives from which applications can build workflows, plus standalone C# and Java builders for normalized choreography declarations. It does not yet register or merge those fragments, compare them with topology and observations, or supply a saga repository, saga state-machine runtime, or compensation engine. Documentation and dashboards must not imply those capabilities merely because the models reserve room for them.
...
The Dashboard can eventually turn these abstractions into two related workflow views. For choreography, it reconstructs decentralized reactions from declarations and observed causal message paths and makes uncertainty visible. For orchestration, the saga runtime can report authoritative instance state, transitions, pending timeouts, faults, and completion. Shared maps and timelines should let developers follow the same business story in either model, while the UI clearly labels inferred choreography evidence differently from persisted orchestration state.
