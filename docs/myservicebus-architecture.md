...
Distributed workflow coordination is an evolving preview product area with two distinct styles:

The problem usually first appears as choreography. Services publish facts and add local consumers that react and emit further messages, often inline with the application code that performs the reaction. Each component can be straightforward while the broader workflow becomes implicit—a web of contracts, producers, consumers, states, and branches spread across independently deployed services. A choreography declaration can enrich those local reactions with stable workflow, step, ownership, expectation, and monitoring context, but it does not relocate the decisions or become an executable coordinator. Teams then need a way to express the intended reactions, find outcomes they forgot to handle, distinguish optional silence from a stalled process, and understand cycles or amplification without introducing a hidden coordinator.

For some workflows, that discovery shows that process knowledge should have one explicit owner. Orchestration defines that broader knowledge separately in a coordinator—typically a saga or state machine hosted by one coordinating service—which receives messages from several services and directs the next work through messages. This makes progress and next actions explicit, but it also introduces the harder runtime responsibilities of durable state, correlation, concurrency, timeouts, compensation, retries, and recovery.

- **Choreography is decentralized workflow coordination:** participants react to messages according to their own local rules, often declaring sends or publications alongside the code that makes each decision, and collectively produce the broader process. Applications can already compose basic choreography from publish and consume primitives. MyServiceBus declarations add context to those reactions for validation and monitoring without owning their execution. The current Dashboard includes declared workflows, bounded reconstructed runs, exact-evidence flow diagrams, and completeness-gated output comparison; broader cycle, pattern, and amplification diagnostics remain future work.
...
The current runtime supplies the messaging, correlation, scheduling, outbox, retry, fault, and monitoring primitives from which applications can build workflows. C# and Java can also build, validate, register, and export normalized choreography fragments through topology and inspection metadata. The monitoring service merges those fragments, projects declared contract connections, reconstructs selectable bounded workflow runs, and compares declared outputs with exact causal evidence. Matching C# and Java saga DSLs and executors run through provider-neutral repository transactions, validate declared correlation, concurrency, durability, outbox, and final-deletion requirements against provider capabilities, register their events on one receive endpoint, dispatch sends and publications through the active consume context, export normalized state-machine definitions, and emit payload-free lifecycle observations after repository commit. Both volatile and PostgreSQL repositories are supplied; the durable provider serializes competing deliveries and commits saga state with transactional outbox records. The monitoring service retains separate saga definitions and bounded authoritative instance projections with replica, conflict, state, completion, and delivery-attempt information rather than reinterpreting them as choreography. Shared workflow queries compose references to both models with an explicit implementation kind and lifecycle authority, and the Dashboard links each entry to its kind-specific declaration or run detail. The Aspire environment demonstrates both C#- and Java-owned coordination with cross-language participants and MassTransit peers. This remains an experimental orchestration foundation rather than a complete saga platform: durable timeouts, composite events, request activities, and compensation remain future profiles.

The first workflow investment was the read-only choreography extension defined by [Choreography Modeling and Diagnostics](proposals/choreography-modeling-and-diagnostics.md), followed by the saga runtime. Configured routing, application-declared reactions, and bounded observed flow remain separate graphs. Exact message or causal identity is stronger than correlation-only evidence, and diagnostics expose confidence, freshness, and completeness rather than treating missing observations as proof of business failure.
...
The Dashboard can turn these abstractions into two related workflow views. For choreography, the implemented definition map and aggregate evidence overlay should lead to selectable bounded workflow runs: a flow diagram plus an ordered step breakdown showing the participating application and consumer, handler and transit time, emitted messages, retries, and failures while keeping uncertainty visible. For orchestration, the saga runtime can report authoritative instance state, transitions, pending timeouts, faults, and completion. Monitoring retains choreography and saga/state-machine records as separate bus concepts because their identities, lifecycle, and evidence differ. A shared workflow query projects either kind into the common catalog, run list, maps, and timelines used by the Dashboard without forcing them into one storage shape or erasing their kind-specific details.
...
The [C# and Java saga runtimes](proposals/sagas-and-state-machines.md) are MyServiceBus-owned reimplementations based on MassTransit's proven saga architecture, primitives, execution stages, and observable behavior rather than direct MassTransit runtime dependencies. Each client provides an independent library-based state-machine DSL; neither depends on the optional Raven macro. Platform structure differs, while shared fixtures verify equivalent correlation, transition, persistence, concurrency, outbox, fault, and completion outcomes. The remaining orchestration profiles—durable scheduling, requests, composite events, compensation, and richer concurrency choices—must extend the same coherent portable model. Any directly incorporated source must retain its required notices and license treatment. Choreography support remains decentralized rather than introducing an implicit coordinator, and it focuses on making event relationships and unhealthy or incomplete flows observable without pretending that the framework owns application business state.

Language-specific projections may become more expressive than the initial descriptor APIs without redefining this boundary. The shared model is a semantic target, not a demand for identical public surfaces: each client should use its language's type system, functional forms, annotations, generation facilities, and established conventions to make authoring clearer. MyServiceBus can borrow strong ideas from multiple messaging, workflow, and state-machine APIs without preserving their historical API baggage. The experimental [Raven saga DSL](proposals/raven-saga-dsl.md) already compiles a concise state-machine declaration into the same .NET definition and runtime behavior; a later choreography DSL may lower into the portable choreography fragments produced by C# and Java. Raven syntax and compiler artifacts remain outside persisted state, wire contracts, and normalized topology, and the experiment follows rather than defines the shared semantics.
