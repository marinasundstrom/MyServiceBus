...
The current runtime supplies the messaging, correlation, scheduling, outbox, retry, fault, and monitoring primitives from which applications can build workflows. C# and Java can also build, validate, register, and export normalized choreography fragments through topology and inspection metadata. The monitoring service merges those fragments, projects declared contract connections, reconstructs selectable bounded workflow runs, and compares declared outputs with exact causal evidence. Matching low-level C# and Java saga definitions and volatile in-memory executors now prove the fundamental orchestration behavior, but they are not yet registered with receive endpoints, dispatching captured operations through the bus, or supported as a complete saga feature. No durable saga repository or compensation engine is currently supplied.
