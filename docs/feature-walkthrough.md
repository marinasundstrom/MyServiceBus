...
The repository's mixed Aspire sample declares three progressively richer views: `sample-local-order-observation` is one C# terminal reaction, `sample-order-submission` combines C# and Java reactions into decentralized fan-out, and `sample-fulfillment-handoff` follows a linear C# → Java → C# chain. Start the latter with `POST /workflows/fulfillment` on the C# app. Every declaration matches real consumers and emitted messages; none invents a coordinator. Run the Aspire stack and open the MyServiceBus monitoring Dashboard to inspect the merged declarations and their reconstructed activity. `Live activity`, `terminal observed`, and `no recent activity` are evidence labels rather than authoritative choreography lifecycle states.
