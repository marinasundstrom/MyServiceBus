...
### RabbitMQ Transport
  ...
  - `RabbitMqSendTransport` uses the selected serializer content type: `application/vnd.masstransit+json` for the default envelope, `application/json` for neutral Raw JSON and the explicitly selected NServiceBus JSON profile, or `application/vnd.masstransit+bson` when the optional BSON module is selected.
