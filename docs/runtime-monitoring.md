...
Request/response remains a first-class **exchange projection over two related messages**, not another wire primitive or a duplicate store. The request and response retain their own message identities and lifecycle pages; explicit request identity lets the monitoring service pair them and calculate requester, responder, pending or completed state, round-trip duration, and failures. A focused exchange inspector can therefore present an HTTP-inspector-like request and response view—including separately disclosed bodies and safe envelope metadata—while linking back to each constituent message. When request metadata is not exported, the collector must leave the exchange unpaired rather than infer an authoritative relationship from payloads or timing.
...
The orchestration-facing queries follow that rule. `GET /api/monitoring/v1/sagas` returns registered state-machine definitions, endpoint attachments, replica counts, availability, and definition conflicts. `GET /api/monitoring/v1/sagas/instances` returns recently observed instances and their committed transitions, with optional state-machine and status filters. The clients emit these observations only after the repository transaction commits; failures are recorded as failed deliveries without inventing a state transition, and saga payload fields are never exported. This first instance projection is bounded by the in-memory observation window and is not yet durable history. Neither query contains choreography fragments.
