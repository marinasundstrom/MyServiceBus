...
The orchestration-facing queries follow that rule. `GET /api/monitoring/v1/sagas` returns registered state-machine definitions, endpoint attachments, replica counts, availability, and definition conflicts. `GET /api/monitoring/v1/sagas/instances` returns retained instances and their committed transitions, with optional state-machine and status filters. The clients emit these observations only after the repository transaction commits; failures are recorded as failed deliveries without inventing a state transition, and saga payload fields are never exported. The monitoring repository merges current evidence into one bounded instance projection per state-machine, application, and correlation identity. The PostgreSQL history provider persists and restores those projections under the configured retention period, while the in-memory provider remains volatile. Neither query contains choreography fragments.

Two composition queries provide shared navigation without becoming new storage authorities. `GET /api/monitoring/v1/workflows` catalogs declared choreography and saga definitions with an explicit `kind` and `lifecycleAuthority`. `GET /api/monitoring/v1/workflow-runs/index` returns common run summaries for filtering and sorting. Each catalog entry opens its own kind-specific declaration page, and each run summary carries a kind-specific detail identity: choreography opens the retained causal reconstruction, while saga opens the bounded committed-transition instance view. The projection does not copy saga state into choreography records or upgrade reconstructed choreography evidence into authoritative lifecycle state.
...
```bash
dotnet add package Sundstrom.MyServiceBus.Monitoring --version 0.1.0-preview.11
```
...
```groovy
implementation 'io.github.marinasundstrom.myservicebus:myservicebus-monitoring:0.1.0-preview.11'
```
...
The provider applies its schema migration at startup and stores the latest metadata, heartbeat, scheduled-work, recurring-definition, and tracked-job snapshots per bus identity plus deduplicated observation batches, reconstructed choreography runs, and authoritative saga-instance projections as JSONB. On restart, it restores those latest authoritative snapshots, rebuilds the current 15-minute observation window without making restored records appear newly ingested, and restores choreography runs and saga instances retained within the configured storage period. The seven-day default bounds observation batches and both kinds of workflow drill-down. It does not turn the other latest-state snapshots or metric buckets into historical time series.
