...
Scheduled work should become another application-focused view. It should list bounded operational metadata for scheduled messages and provider-defined jobs: application, provider, safe message or job type, due time, destination or intent, status, attempt, and last failure category. The initial status vocabulary should preserve provider truth while normalizing common states such as `Pending`, `Leased` or running, `Dispatched` or completed, `Cancelled`, and `Dead` or failed. Message bodies, arbitrary headers, serialized callbacks, and application payloads remain excluded.
