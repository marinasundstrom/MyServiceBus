...
Active implementation proposal. The portable version 1 behavior is defined in the [Saga State-Machine Behavior specification](../specs/saga-state-machine-behavior.md), and matching low-level runtimes plus native C# and Java DSL foundations lower to the same canonical definition and execution sequence. Both runtimes use public provider-neutral repository transactions and validate declared requirements against provider capabilities. Built-in profiles now include the explicitly volatile in-memory repository and matching PostgreSQL repositories with pessimistic identity locking, durable JSON state, final deletion, and transactional outbox capture. Experimental registration attaches declared events to one bus endpoint, dispatches outgoing work through the active consume context, exports the normalized definition through topology and inspection, and emits payload-free lifecycle observations after repository commit. The collector durably retains dedicated definition and bounded instance-transition saga projections plus shared workflow catalog and run-index projections. The Dashboard renders declared state machines and committed run paths. Focused tests, matching restart/rollback/concurrency recovery gates, and a mixed C#/Java Aspire order workflow demonstrate the vertical slice. Durable scheduling and requests, richer branching and failure semantics, broader recovery evidence, and API refinement remain before this is a complete saga feature.
