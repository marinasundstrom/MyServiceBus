...
6. Assemble exact bounded causal chains into a monitoring-owned workflow-run projection, including step/consumer mapping, separate handler and transit duration, outgoing operations, retries, and failures.
7. Add the workflow-run flow diagram and ordered step drill-down to the focused Dashboard view; the declaration map, cross-language sample, and exact aggregate evidence overlay are implemented.
8. Implement broader graph comparison and diagnostics in the monitoring service with completeness gates. The first exact-only reaction overlay and coverage snapshot are implemented; inferred absence and behavioral diagnostics remain.
