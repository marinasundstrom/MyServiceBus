...
**Current status:** foundation in progress. MyServiceBus has supporting messaging and monitoring primitives plus matching C# and Java choreography declaration, validation, registration, topology, inspection, and metadata export. The monitoring service merges those declarations across applications and replicas into a read-only query with freshness and definition-conflict evidence, and the Dashboard renders the resulting definition map by application, trigger, output, and downstream participant. A separate bounded projection now overlays exact causal send and publish counts with dropped-observation and participant-availability coverage, without treating silence as failure. The mixed Aspire environment provides an executable C# and Java declaration sample over its real order consumers. The next Dashboard-oriented MVP priority is a monitoring-owned, bounded workflow-run reconstruction with a flow diagram and step breakdown showing participant or consumer mapping, handler and transit duration, outgoing messages, retries, and failures. Broader declared-versus-observed diagnostics, timing and multiplicity evaluation, and pattern discovery follow that drill-down, and there is no supported saga repository, saga state-machine runtime, compensation engine, or choreography-specific lifecycle model.
