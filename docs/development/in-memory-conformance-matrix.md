...
| 4 | Request/response correlation, timeout, cancellation, and fault response | `GenericRequestClientTests`, including timeout/cancellation distinction; `InMemoryHarnessDiTests.Should_resolve_request_client`; `RequestFaultExceptionTests` | `GenericRequestClientPolicyTest`; `InMemoryHarnessDiTest.request_client_round_trip`; `RequestClientFaultTest`; `RequestClientHeaderTest` | **Partial** | Assert correlation identifiers end to end in matching local-runtime scenarios. |
...
1. complete request correlation scenarios
