...
```java
dependencies {
    implementation "io.github.marinasundstrom.myservicebus:myservicebus:0.1.0-preview.10"
    annotationProcessor "io.github.marinasundstrom.myservicebus:myservicebus-processor:0.1.0-preview.10"
}
```
...
The preview-pinned CI smoke targets `net11.0`, enables `runtime-async=on`, and forces both an ordinary `IConsumer<TMessage>` implementation and a generated C# named-union method consumer to suspend at `Task.Yield()`. It also sets `EnableNet11RuntimeAsyncTarget=true`, which rebuilds `MyServiceBus.Abstractions` and the core `MyServiceBus` runtime for `net11.0` with Runtime Async before publishing the native executable. The generated catalog registers one direct adapter per concrete union case, excludes runtime union discovery from the native path, and verifies both cases through the rebuilt mediator pipeline.
