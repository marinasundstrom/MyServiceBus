...
- **Share JVM behavior, project JVM APIs**: Keep topology, definitions, delivery, and transports in a common JVM core. Java and Kotlin should expose separate application-facing projections when futures versus coroutines, class tokens versus reified types, overload resolution, nullability, or framework conventions make one surface unnatural in the other language.
