...
- **Cancellation tokens** – Routine Java operations provide tokenless overloads. Advanced overloads accept a read-only `CancellationToken` to propagate cooperative cancellation through futures, filters, retries, consumers, and transports. `CancellationToken.none()` is used internally when no cancellation policy is supplied; callers create a `CancellationTokenSource` only when they need to request cancellation.
