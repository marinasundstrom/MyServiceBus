...
The trusted policy owner must own all eight package IDs:
...
2. Wait for the workflow to reach Maven Central state `PUBLISHING` or `PUBLISHED`. It tests all Java modules, creates signed publications, verifies a clean consumer, uploads one bundle containing all twelve artifacts, and waits for Central to validate and accept it. Publication then continues asynchronously in Central.
3. Run **Publish NuGet preview** and select the same release tag.
4. Confirm that all eight NuGet packages and symbol packages were accepted.
