...
After an error copy succeeds, the filter annotates the original exception with
portable settlement metadata and rethrows that same exception. A receive
transport can then acknowledge or complete the original broker delivery
without changing the terminal exception observed by upstream filters. If the
copy fails, no successful-move marker is present and the transport retains its
normal abandon, unlock, or negative-acknowledgement behavior.
