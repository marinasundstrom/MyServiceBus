...
- The durable recurring provider now supplies authoritative application outcomes through correlated tracked jobs. The monitoring service still exports definitions only, so the dashboard must not show occurrence completion until retained occurrence and job projections are added.
- The tracked `JobConsumer` MVP is specified in [Job Consumers](job-consumers.md). It adds progress, retry, cancellation, concurrency, and authoritative execution status. Explicit interface handlers come first; consumer-method-style declarations and matching generated registration through the C# source generator and Java annotation processor follow through the same normalized descriptors and pipeline.
