...
- A future `JobConsumer`-style abstraction may add long-running-work conveniences such as progress, retry, cancellation, and execution status. Job handlers should support explicit interfaces, consumer-method-style declarations, manual registration, and matching generated registration through the C# source generator and Java annotation processor. All paths produce the same runtime descriptors and pipeline. This execution layer is not a prerequisite for the scheduling primitives.
