...
| Recurring jobs MVP | `IRecurringJobScheduler`, provider/source seams, in-memory and built-in durable providers | `RecurringJobScheduler`, provider/source seams, in-memory and built-in durable providers | Fixed intervals, revisions, controls, capped misfires, transactional tracked-job materialization, restart recovery, monitoring, and bidirectional shared-PostgreSQL materialization are verified. In-memory tracked promotion, occurrence-history monitoring, cron, and third-party adapters remain open. |
