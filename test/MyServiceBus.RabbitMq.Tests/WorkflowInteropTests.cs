...
namespace MyServiceBus.RabbitMq.Tests
{
    ...
    public class WorkflowInteropTests
    {
        ...
        [CrossLanguageFact]
        public async Task MassTransit_and_CSharp_participate_in_a_Java_saga_workflow()
        {
            await using var container = new RabbitMqBuilder("rabbitmq:4.1.8-alpine").Build();
            await container.StartAsync();

            var orderId = Guid.NewGuid();
            var participantQueue = $"workflow-csharp-{Guid.NewGuid():N}";
            var connectionUri = new Uri(container.GetConnectionString());
            var credentials = connectionUri.UserInfo.Split(':', 2);
            var services = new ServiceCollection();
            services.AddLogging();
            services.AddServiceBus(configurator =>
            {
                configurator.AddConsumer<CSharpWorkflowParticipant, TestApp.InteropWorkflowWorkRequested>(
                    participantQueue);
                configurator.UsingRabbitMq((_, rabbit) => rabbit.Host(
                    container.Hostname,
                    container.GetMappedPublicPort(5672),
            ...
            Process? javaPeer = null;
            await hostedService.StartAsync(CancellationToken.None);
            await massTransitBus.StartAsync();
            try
            {
                javaPeer = JavaInteropPeer.Start(
                    container,
            ...
            {
                if (javaPeer is { HasExited: false })
                    javaPeer.Kill(entireProcessTree: true);
                await massTransitBus.StopAsync();
                await hostedService.StopAsync(CancellationToken.None);
            }
        ...
        public sealed class CSharpWorkflowParticipant : MyServiceBus.IConsumer<TestApp.InteropWorkflowWorkRequested>
        {
            public Task Consume(MyServiceBus.ConsumeContext<TestApp.InteropWorkflowWorkRequested> context)
                => context.Publish(new TestApp.InteropWorkflowWorkCompleted { OrderId = context.Message.OrderId });
        }
