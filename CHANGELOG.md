...
- Defined recurring jobs as durable, revisioned definitions that create independently tracked occurrences, added matching validated C#/Java contracts and an overridable provider boundary, and generalized scheduling durability across message and recurring providers. The built-in in-memory provider supplies a volatile development baseline, with fixed-interval misfire behavior verified by shared cross-language fixtures. PostgreSQL schema version 4 and matching .NET/Java provider implementations persist idempotent definitions and pause/resume/remove controls for the built-in durable provider. The model includes explicit cadence dialects, capped misfire and overlap policies, dispatch-only initial status, transactional materialization, monitoring coverage, distinct .NET-native, Java-native, and mixed-language provider profiles, and a later JobConsumer execution boundary.
