...
- Defined a MassTransit-familiar, cross-language tracked-job surface with `IJobConsumer<TJob>` / `JobConsumer<TJob>`, job and attempt state, submission and control clients, progress, and monitoring sources. Added matching C# and Java explicit registration plus volatile executors that prove immediate and delayed submission, per-consumer concurrency, timeout, automatic and manual retry, cooperative cancellation, progress, and attempt history. The first C# PostgreSQL worker now persists intent and attempts, uses fenced leases and heartbeats, survives provider restart, and exposes durable progress and history; Java parity, recurring-occurrence promotion, monitoring export, and Aspire/dashboard proof remain before the job-consumer MVP is complete.
