...
- Added the first Kotlin-native JVM facade with a Kotlin-owned configuration DSL, reified consumer, handler, and transport registration, typed dependency-injection helpers, coroutine-awaitable messaging operations, suspending consumers, and typed suspending request handlers with bidirectional cancellation. Kotlin's projected handler contract also hardened the shared Java mediator's inherited generic response discovery. A runnable RabbitMQ and mediator sample, focused Kotlin tests, staged package consumer, Maven publication wiring, and three-language website samples establish the compatibility path for future KSP support.
