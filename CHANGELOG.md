...
- Added server-side pagination and retained-result counts to the Dashboard message and failure indexes, with URL-backed message search and status filters that remain applied while paging. Workflow runs and potentially large current-state tables for endpoints, replicas, scheduled work, recurring jobs, and tracked jobs now share the same reusable list pager.
