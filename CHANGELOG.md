...
- Added the first Kotlin-native JVM facade with a Kotlin-owned configuration DSL, reified consumer, handler, and transport registration, typed dependency-injection helpers, coroutine-awaitable messaging operations, suspending consumers, and typed suspending request handlers with bidirectional cancellation. An async-shape-neutral JVM `ResultHandler` metadata contract lets Kotlin expose `suspend fun handle` without inheriting Java futures, while the projected handler work also hardened the shared mediator's inherited generic response discovery. A runnable RabbitMQ and mediator sample, focused Kotlin tests, staged package consumer, Maven publication wiring, and three-language website samples establish the compatibility path for future KSP support.
