...
- Added the first Kotlin-native JVM facade with a Kotlin-owned configuration DSL, reified consumer, handler, transport, and request-client creation, coroutine-awaitable messaging operations, typed suspending request handlers, and exhaustive multiple-response results with bidirectional cancellation. Kotlin now owns familiar `Consumer` and `ConsumeContext` projections whose suspending `publish`, `send`, `respond`, `forward`, and `respondFault` members avoid collisions with Java's future-returning overloads; the former `SuspendConsumer` name remains as a source-migration alias. An async-shape-neutral JVM `ResultHandler` metadata contract lets Kotlin expose `suspend fun handle` without inheriting Java futures, while the projection work also hardened the shared mediator's inherited generic response discovery and evolved Java's two-response result into nominal sealed record cases. A runnable RabbitMQ and mediator sample, focused Kotlin tests, staged package consumer, Maven publication wiring, and three-language website samples establish the compatibility path for future KSP support.
